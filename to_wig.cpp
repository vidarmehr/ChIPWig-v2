			  ...
			  if(Dumm == 0)
               ...
               if((DiffVal+PreVal) == -1 )
               {
				   ...
				   fprintf(fpWritebf, "%ld\t%.4f\n", DiffLoc+PreLoc, C1);
                        ...
                        else if ((DiffVal+PreVal) == 0)
                            fprintf(fpWritebf, "%ld\t%d\n", DiffLoc+PreLoc, 0);


			   else if( (DiffVal+PreVal)% 1000 == 0)
			  	{
			  	//fgetpos(fpWriteb,&position);
			  	fprintf(fpWritebf, "%ld\t%d\n", DiffLoc+PreLoc, (DiffVal+PreVal)/1000 );
			  }
			 else if ((DiffVal+PreVal) % 100 == 0)
			  {
				//fgetpos(fpWriteb,&position);
                 fprintf(fpWritebf,"%ld\t%.1f\n", DiffLoc+PreLoc, (DiffVal+PreVal)/1000.0);
			  }

int partial_unblockL(char *argv[], char *str, long int Start, long int End)
{
	long int sz,sz1,sz2;
	char * buffer;
	char FileName[200];
			...
			if(Flag_start_file == 1)
			{
				while(!feof(fpReadBlock))
				{
			        ...
			        if(tmp >='0' && tmp<='9')
			        {
					   ungetc(tmp,fpReadBlock);
					   fscanf(fpReadBlock,"%ld %f\n", &Loc, &Val);
					   int integ_value= Val * 100;
				       if(integ_value % 100 == 0)
				       {
					      fprintf(fpWriteWig,"%ld %d\n", Loc, integ_value/100);
				       }
				       else if(integ_value % 10 == 0)
				       {
					      fprintf(fpWriteWig,"%ld %.1f\n", Loc, integ_value/100.0);
				       }
				       else
                          fprintf(fpWriteWig,"%ld %.2f\n", Loc, Val);
				   }
...
int flag2 = 0;
	       ...
	       if(End>ArrayLocations[Size-1])
		    {
		     ...
		     {
				 int in_value= ArrayValues[j] * 100;
				 if(in_value % 100 == 0)
				 {
					 fprintf(fpWriteSeq,"%ld %d\n", ArrayLocations[j], in_value/100);
				 }
				 else if(in_value % 10 == 0)
				 {
					 fprintf(fpWriteSeq,"%ld %.1f\n", ArrayLocations[j], in_value/100.0);
				 }
				 else
	...
	NextChrom=0;
		...
		fprintf(fpSS, "Min_Value=%.2f, Max_Value= %.2f, Mean= %lf ", Min_Value , Max_Value , Mean);
		fclose(fpSS);

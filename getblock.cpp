        ...
        while(!feof(fpRead))
	{
		...
		while(NumberOfData<exp2(BlockSize) && !feof(fpRead) ){
		   ...
		   if  (tmp >= '0' && tmp <= '9') {
				...
				Map[NumberOfActualData] = (Value*1.0) /1000;
			        ++NumberOfData;
				++NumberOfActualData;
				if(Value < Min_Value)
			    	Min_Value = Value;
				if(Value > Max_Value)
			    	Max_Value = Value;
				SumValue = SumValue + ((Value*1.0) /1000);
		...
		fprintf(fpWriteSummary, "%.3f %.3f %.4lf %.4lf ", (Min_Value *1.00)/1000, (Max_Value *1.00)/1000, Mean, std);
